...
cd "$repo_root"
qsub -t "1-${expected_tasks}:1" -v "UV_BIN=$uv_bin,PILOT_TRAIN_DIR=$train_dir,PILOT_EVAL_DIR=$eval_dir,PILOT_OUTPUT_DIR=$output_dir,PILOT_CONFIG_TEMPLATE=$config_template,PILOT_LAMBDA_GRID=$lambda_grid,PILOT_RUN_NAME=$run_name" qsub_pilot.sh
